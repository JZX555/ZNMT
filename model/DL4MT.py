...
# from module.gpu_mem_track import MemTracker
...
class Encoder(nn.Module):
    def __init__(self,
        ...
        self.gru = RNN(type="gru", batch_first=True, input_size=input_size, hidden_size=hidden_size,
                       bidirectional=True)
        self.extention_fuc = None

    def set_extention_emb_fuc(self, extention_fuc, extention_size):
        self.extention_fuc = extention_fuc

        if extention_size > 0:
            self.linear_extention = nn.Linear(in_features=self.d_word_vec + extention_size, out_features=self.d_model)
    ...
    def get_embeddings(self, seqs):
        ...
        # concencate with extention embedding, and transfer the last dim to d_model
        if self.extention_fuc is not None:
            emb = self.extention_fuc(emb, seqs)
            emb = self.linear_extention(emb)
        
        return emb

    def forward(self, xs, xlengths):
        """
...
class DL4MT(nn.Module):
    def __init__(self, config, src_vocab, tgt_vocab, use_gpu=True):
        ...
        self.encoder = Encoder(src_vocab, input_size=config.embed_size, hidden_size=config.hidden_size)
    ...
    def batch_beam_search(self, x, lengths, beam_size=5, max_steps=150, alpha=-1.0, log_probs=True):
        batch_size = x.size(0)
        ...
        for t in range(max_steps):
            ...
            next_scores = -self.generator(logits, log_probs=log_probs)  # [B * Bm, N]
