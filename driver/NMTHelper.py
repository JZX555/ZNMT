...
class NMTHelper(object):
    ...
    def transfer_embeddings(self, src_vocab:Vocabulary, tgt_vocab:Vocabulary, embedding:Embeddings):
        new_weight = new_weight = torch.empty_like(embedding.embeddings.weight)

        # reset clone weight
        Init.embedding_init(new_weight)
    ...
    def load_extention_vocab(self):
        ...
        for name, path in vocabs.items():
            ...
            embedding = Embeddings(num_embeddings=vocab.max_n_words, 
    ...
    def pair_data_variable(self, batch):
        ...
        src_words = torch.zeros([batch_size, max_src_length], dtype=torch.int64, requires_grad=False).fill_(self.src_pad)
        tgt_words = torch.zeros([batch_size, max_tgt_length], dtype=torch.int64, requires_grad=False).fill_(self.tgt_pad)
        ...
        if fusion_list is not None:
            for name in fusion_list:
                if name in self.extention_embeddings:
                    ...
                    '''
                    emb = torch.cat([emb, self.extention_embeddings[name](seqs_x)], dim=-1)
    ...
    def compute_forward(self, seqs_x, seqs_y, xlengths, normalization=1.0):
        ...
        mask = y_label.ne(NMTVocab.PAD)
        pred = self.model.generator(dec_outs).max(2)[1]  # [batch_size, seq_len]
